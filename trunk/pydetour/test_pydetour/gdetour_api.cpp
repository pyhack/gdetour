...
#ifdef _DEBUG
	HMODULE gd = LoadLibrary("_detour_d.pyd");
	if (gd == NULL) {
		MessageBox(0,"Could not load _detour_d.pyd", "Error", 0);
		return;
	}
#else
	HMODULE gd = LoadLibrary("_detour.pyd");
	if (gd == NULL) {
		MessageBox(0,"Could not load _detour.pyd", "Error", 0);
		return;
	}
#endif
	this->imp_add_detour = (add_detour_func) GetProcAddress(gd, "add_detour");
	if (this->imp_add_detour == NULL) {
		MessageBox(0,"Could not find add_detour in _detour.pyd", "Error", 0);
	}
	this->imp_run_python_file = (run_python_file_func) GetProcAddress(gd, "run_python_file");
	if (this->imp_run_python_file == NULL) {
		MessageBox(0,"Could not find run_python_file in _detour.pyd", "Error", 0);
	}
}
...
int gdetour::run_python_file(char* filename) {
	if (this->imp_run_python_file == NULL) {
		MessageBox(0,"Could not find imp_run_python_file in _detour.pyd", "Error", 0);
		return false;
	}
